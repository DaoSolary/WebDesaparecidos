...
export function HeatmapMap({ cases, provinceStats }) {
            ...
            if (provinceStats && provinceStats.length > 0) {
                ...
                provinceStats.forEach((stat) => {
                    L.marker([stat.lat, stat.lng])
                        .addTo(map)
                        .bindPopup(`<strong>${stat.province}</strong><br>` +
    ...
    return (_jsxs("div", { className: "rounded-lg border border-slate-200 bg-white p-4", children: [_jsxs("div", { className: "flex items-center gap-2 mb-4", children: [_jsx(MapPin, { className: "h-5 w-5 text-blue-600" }), _jsx("h3", { className: "font-semibold text-slate-900", children: "Mapa de Calor" })] }), _jsx("div", { ref: mapRef, className: "w-full h-96 rounded-lg bg-slate-100" }), _jsx("p", { className: "mt-2 text-xs text-slate-500", children: provinceStats && provinceStats.length > 0
